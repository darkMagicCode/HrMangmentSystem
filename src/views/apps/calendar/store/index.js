...
export const updateAllFilters = createAsyncThunk('appCalendar/updateAllFilters', async (value, { dispatch }) => {
  if (value === true) {
    await dispatch(fetchEvents(['Work', 'Business', 'Employee', 'Holiday', 'ETC']))
  } else {
...
export const appCalendarSlice = createSlice({
  ...
  initialState: {
    events: [],
    selectedEvent: {},
    selectedCalendars: ['Work', 'Business', 'Employee', 'Holiday', 'ETC']
  },
  reducers: {
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(updateAllFilters.fulfilled, (state, action) => {
        ...
        let selected = []
        if (value === true) {
          selected = ['Work', 'Business', 'Employee', 'Holiday', 'ETC']
        } else {
          selected = []
        }
